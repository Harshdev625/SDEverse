...
const AdminAnalytics = () => {
  ...
  const activeUsers =
    adminAnalytics?.platformMetrics?.activeUsersLast30Days || 0;
  ...
  const statCards = [
    ...
    },
    {
      ...
      trend: `${
        Math.round(
          ((adminAnalytics?.platformMetrics?.totalAdmins || 0) / totalUsers) *
            100
        ) || 0
      }% of total`,
    },
    {
      ...
      trend: `${parseFloat(
        adminAnalytics?.userEngagement?.avgAlgorithmsPerUser || 0
      ).toFixed(2)} per user`,
    },
    {
      title: "Proposals",
      ...
      trend: `${
        adminAnalytics?.qualityMetrics?.proposalApprovalRate || 0
      }% approved`,
    },
    {
      title: "Comments",
      ...
      trend: `${parseFloat(
        adminAnalytics?.userEngagement?.avgCommentsPerUser || 0
      ).toFixed(2)} per user`,
    },
    {
      title: "Active Users",
  ...
  const engagementMetrics = [
    {
      ...
      value:
        adminAnalytics?.userEngagement?.userActivityBreakdown
          ?.algorithmContributors || 0,
    ...
    return (
      <div
        className={`flex justify-center items-center h-screen ${
          themeMode === "dark" ? "bg-gray-950" : "bg-gray-50"
        }`}
      >
        <Loader size="large" />
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-8">
          <div className="mb-8">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              >
                {adminAnalytics?.platformMetrics?.totalAlgorithms || 0}{" "}
                algorithms
              </span>
              <span
                className={`px-3 py-1 rounded-full text-sm ${
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              ...
              >
                ...
                >
                  {dailyData.length
                    ? Math.round(
                        dailyData.reduce((sum, day) => sum + day.newUsers, 0) /
                          dailyData.length
                      )
                    : 0}
                </p>
            ...
            <div className="overflow-x-auto">
              <table
                className={`min-w-full divide-y ${
                  themeMode === "dark" ? "divide-gray-800" : "divide-gray-200"
                }`}
              >
                <thead
                  className={
                ...
                <tbody
                  className={`divide-y ${
                    themeMode === "dark"
                      ? "bg-gray-900/50 divide-gray-800"
                  ...
                  {rolePercentages.map((role, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <div
                            className={`w-24 rounded-full h-2 mr-3 ${
                              themeMode === "dark"
                                ? "bg-gray-700"
                                : "bg-gray-200"
                            }`}
            ...
            >
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
                    data={
                      adminAnalytics?.contentDistribution
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    >
                      {adminAnalytics?.contentDistribution?.proposalsByStatus?.map(
                        (entry, index) => (
                          <Cell
                            key={`cell-${index}`}
                            fill={PALETTE[index % PALETTE.length]}
