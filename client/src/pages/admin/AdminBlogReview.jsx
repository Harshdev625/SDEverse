...
const AdminBlogReview = () => {
  ...
  const { drafts, loading, error, pagination } = useSelector(
    (state) => state.blog
  );
  ...
  const handlePublish = async () => {
    ...
    dispatch(
      getDraftBlogs({ page: currentPage, limit: 10, search: searchQuery })
    );
  };
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-lg text-gray-600 dark:text-gray-400">
                  {drafts.length} draft{drafts.length !== 1 ? "s" : ""} awaiting
                  review
                </p>
              </div>
            ...
            {drafts.length === 0 ? (
              ...
              >
                ...
                <p className="text-gray-500 dark:text-gray-400">
                  All blogs are up to date. Check back later for new
                  submissions.
                </p>
              </motion.div>
            ) : (
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                      ...
                      <tbody className="bg-white/50 dark:bg-gray-800/50 divide-y divide-gray-200 dark:divide-gray-700">
                        {loading
                          ...
                          : drafts.map((blog) => {
                              const status =
                                blog.reviewStatus || blog.status || "draft";
                                    ...
                                    <div className="flex items-center gap-3">
                                      <img
                                        ...
                                        src={
                                          ...
                                          `https://ui-avatars.com/api/?name=${blog.author.username}&background=random`
                                        }
                                        alt=""
                                      />
                                      <div>
                  ...
                  {pagination && pagination.totalPages > 1 && (
                    <div className="bg-white/50 dark:bg-gray-800/50 px-6 py-4 flex items-center justify-between border-t border-gray-200 dark:border-gray-700">
                      <p className="text-sm text-gray-700 dark:text-gray-400">
                        ...
                        to{" "}
                        <span className="font-medium">
                          {Math.min(currentPage * 10, pagination.totalDrafts)}
                        </span>{" "}
                        of{" "}
                        <span className="font-medium">
                          {pagination.totalDrafts}
                        </span>{" "}
                        drafts
                      </p>
